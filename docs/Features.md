...
Nodes are the machines that run game servers. Each node runs the Arclight daemon, which the panel communicates with.
...
The marketplace tab lists community addons from the `arclighted/addons` registry. You can install addons with one click. The panel clones the repository, runs `pnpm install` and `pnpm run build`, and streams the output back to the browser.
...
Available permission scopes: `arclight.api.servers.*`, `arclight.api.users.*`, `arclight.api.nodes.*`, `arclight.api.settings.*`.

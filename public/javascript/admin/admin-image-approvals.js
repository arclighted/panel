  ...
  window.ALMount(function () {
    ...
    function removePendingCard(id, card) {
      ...
      const remaining = list
        ? Math.max(0, list.querySelectorAll(".pending-card").length - 1)
        : 0;
    ...
    document.querySelectorAll("[data-approve-image]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          const response = await fetch("/admin/images/approve/" + id, {
            method: "POST",
          });
    ...
    document.querySelectorAll("[data-reject-image]").forEach((btn) => {
      btn.addEventListener("click", () => {
        openReject(
          btn.dataset.rejectImage,
          btn.closest("div.rounded-xl")?.querySelector("h3")?.textContent || "",
        );
      });
    });

(function () {
  ...
  (function () {
    ...
    function applyThemeFromForm() {
      ...
      var light = formAppearance.querySelector(
        'input[name="lightTheme"]:checked',
      );
      var dark = formAppearance.querySelector(
        'input[name="darkTheme"]:checked',
      );
    ...
    function selectThemeRadio(name, value) {
      ...
      if (changed) {
        var checked = formAppearance.querySelector(
    ...
    function resetAppearanceForm() {
      ...
      document
        .querySelectorAll("#panel-appearance img")
    ...
    function snapshotForms() {
      ...
      document
        .querySelectorAll(
          "#panel-servers input, #panel-security input, #panel-security select, #panel-security textarea",
        )
        .forEach(function (el) {
          formSnapshot.push({
            el: el,
      ...
      document
        .querySelectorAll(
          "#panel-servers [data-format-switcher], #panel-security [data-format-switcher]",
        )
    ...
    function banRowHtml(ip) {
      return (
        ...
        window.escHtml(ip) +
        "</span>" +
        '<button type="button" class="unban-btn text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 transition inline-flex items-center gap-1.5" data-ip="' +
        window.escAttr(ip) +
        '">' +
        (window.alIcon
    ...
    function showEmptyBanList(list) {
      if (!list) return;
      var hasEmpty = Array.prototype.some.call(list.children, function (child) {
    ...
    window.tabHandlers["servers"] = function () {
      ...
      post(
        "/admin/settings/server-policy",
        {
          ...
          onboardingEnabled:
            document.getElementById("onboardingEnabled").checked,
          defaultServerLimit:
    ...
    window.tabHandlers["security"] = function () {
      ...
      Promise.all([
        ...
        .then(function () {
          btn.disabled = false;
          btn.textContent =
            (window.tabLabels && window.tabLabels["security"]) ||
            "Save security";
        })
        .catch(function (err) {
    ...
    document
      ...
      .addEventListener("click", async function (e) {
        ...
        if (!btn) return;
        const d = await window.api("/admin/settings/unban-ip", "POST", {
          ip: btn.dataset.ip,
        });
        if (d && d.success) {

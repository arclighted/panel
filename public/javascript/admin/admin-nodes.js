...
function removeNodeRow(nodeId) {
  const row = document.querySelector(
    '#nodeTable [data-node-id="' + nodeId + '"]',
  );
  if (!row) return;
  al.removeRow(row).then(function () {
    const tbody = document.querySelector("#nodeTable tbody");
    if (tbody && !tbody.querySelector("[data-node-id]"))
      al.showEmpty(tbody, "No nodes yet.", 4);
    syncNodeStats();
  });
}
...
function syncNodeStats() {
  const rows = Array.from(
    document.querySelectorAll("#nodeTable tbody tr[data-node-id]"),
  );
  ...
  setVal(
    "avgNodeInstancesTotal",
    total > 0 ? (instances / total).toFixed(2) : 0,
  );
...
async function deleteNode(nodeId) {
  showConfirmModal(
    "Delete node",
    "This will permanently remove the node. This cannot be undone.",
    async () => {
      try {
        ...
        if (response.ok) {
          ...
          showConfirmModal(
            "Node has servers",
            "There are servers on this node. Delete all servers and remove the node?",
            async () => {
              const r2 = await fetch(
                `/admin/node/${nodeId}?deleteInstance=true`,
                {
                  method: "DELETE",
                  headers: { "Content-Type": "application/json" },
                },
...
function copyCommand(copyBtn, command) {
  navigator.clipboard
    ...
    .catch((error) => {
      ...
      showToast("Couldn't copy the command. Try again.", "error");
    });
}
...
(function () {
  ...
  function updateNodeStatus(nodeId, status) {
    ...
    if (!cell) return;
    var dot = cell.querySelector(
      ".al-dot-online, .al-dot-offline, .al-dot-warning",
    );
    if (!dot) return;
  ...
  function offlineBannerHtml() {
    return (
      ...
      alIcon("refresh-cw", "size-3", { strokeWidth: 1.5 }) +
  ...
  function applyNodeStatuses(nodes) {
    ...
    const anyOffline = nodes.some(function (n) {
    ...
    if (banner && window.al)
      al.patchEl(
        ...
        '<div class="flex items-center gap-3">' +
          alIcon("loader-circle", "w-4 h-4 animate-spin") +
          '<p class="font-medium">Reconnecting…</p></div>',
      );
    fetch("/admin/nodes/list")
      .then(function (r) {
        return r.json();
  ...
  if (window.alRealtime && window.alState) wireNodeStatusRealtime();
    ...
    window.alListener(
      window,
      "al:realtime-ready",
      "admin-nodes-realtime-ready",
      wireNodeStatusRealtime,
    );
  window.alListener(
    document,
    "turbo:before-cache",
    "admin-nodes-realtime-teardown",
    function () {
      stopNodeObservers.splice(0).forEach(function (stop) {
        stop();
      });
    },
  );

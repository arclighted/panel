(function () {
  ...
  let currentSettings = {};
  try {
    currentSettings = JSON.parse(pageData.dataset.modrinthSettings || "{}");
  } catch (e) {
    console.error("Failed to parse Modrinth settings:", e);
  }
  const PROJECT_TYPES = [
    "mod",
    "modpack",
  ...
  function setupProjectTypeButtons() {
    ...
    PROJECT_TYPES.forEach(function (type) {
      ...
      btn.type = "button";
      btn.className =
        "px-3 py-1.5 text-xs border rounded-xl font-medium transition-colors border-neutral-200 dark:border-neutral-600/30 text-neutral-500 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-white hover:border-neutral-400 dark:hover:border-neutral-500 bg-neutral-100 dark:bg-neutral-700/20";
    ...
    try {
      ...
      document.querySelectorAll("[data-type]").forEach(function (btn) {
        ...
        if (disabled.includes(type)) {
          btn.className =
            "px-3 py-1.5 text-xs border rounded-xl font-medium transition-colors bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400";
        } else {
          btn.className =
            "px-3 py-1.5 text-xs border rounded-xl font-medium transition-colors border-neutral-200 dark:border-neutral-600/30 text-neutral-500 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-white hover:border-neutral-400 dark:hover:border-neutral-500 bg-neutral-100 dark:bg-neutral-700/20";
        }
      ...
      console.error("Failed to update project type buttons:", e);
    }
  }
  ...
  function toggleProjectType(type) {
    try {
      ...
      if (idx > -1) settings.disabledProjectTypes.splice(idx, 1);
      else settings.disabledProjectTypes.push(type);
  ...
  function addBlockedProject() {
    ...
    if (!id) {
      showToast("Enter a project ID", "error");
      return;
    }
    try {
      const settings = JSON.parse(document.getElementById("jsonEditor").value);
      ...
      if (settings.blockedProjects.includes(id)) {
  ...
  function removeBlockedProject(id) {
    try {
      ...
      if (idx > -1) {
        settings.blockedProjects.splice(idx, 1);
        updateJsonEditor(settings);
        updateBlockedProjectsList();
        showToast('Removed "' + id + '" from blocked list', "success");
      }
    } catch (e) {
  ...
  function updateBlockedProjectsList() {
    ...
    try {
      ...
      if (!list.length) {
        container.innerHTML =
          '<p class="text-sm text-neutral-400">No blocked projects.</p>';
        return;
      }
      container.innerHTML = list
        .map(function (id) {
          return (
            '<div class="flex items-center justify-between rounded-xl bg-neutral-100 dark:bg-neutral-800/40 border border-neutral-200 dark:border-white/5 px-4 py-2.5">' +
            '<span class="text-sm font-mono text-neutral-700 dark:text-neutral-300">' +
            escapeHtml(id) +
            "</span>" +
            "<button onclick=\"removeBlockedProject('" +
            escapeHtml(id) +
            '\')" type="button" class="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 transition">Remove</button>' +
            "</div>"
          );
        })
        .join("");
    } catch (e) {
      console.error("Failed to update blocked projects list:", e);
    }
  }
  ...
  function applyEasySettings() {
    try {
      ...
      settings.modrinthInstallationWarning =
        document.getElementById("enableWarning").checked;
      settings.warningTitle =
        document.getElementById("warningTitle").value.trim() ||
        "Installation Temporarily Disabled";
      settings.warningMessage =
        document.getElementById("warningMessage").value.trim() || "";
      updateJsonEditor(settings);
  ...
  function saveSettings() {
    ...
    fetch("/modrinth/admin/config", {
      ...
      .then(function (d) {
        if (d.success) showToast("Configuration saved", "success");
  ...
  function updateCharacterCount() {
    document.getElementById("characterCount").textContent =
      document.getElementById("jsonEditor").value.length + " characters";
  ...
  function resetToDefaults() {
    const d = {
      ...
      warningMessage: "Installations are temporarily disabled.",
  ...
  function loadTemplate(type) {
    const t = {
      restrictive: {
        modrinthInstallationWarning: true,
        warningTitle: "Installation Disabled",
        warningMessage: "Mod installation is currently restricted.",
        disabledProjectTypes: ["shader", "resourcepack"],
        blockedProjects: [],
      },
      permissive: {
        modrinthInstallationWarning: false,
      ...
      .then(function (data) {
        ...
        if (data.success && data.data) {
          ...
          el.innerHTML =
            ...
            (f.size
              ? '<span class="text-xs text-neutral-400 dark:text-neutral-600 ml-4">' +
                f.size +
                " bytes</span>"
              : "") +
            (f.lastModified
              ? '<span class="text-xs text-neutral-400 dark:text-neutral-600 ml-2">Modified ' +

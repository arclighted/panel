(function () {
  ...
  function folderMemberUUIDs(folder) {
    return (folder.members || []).map(function (m) {
      return m.serverUUID;
    });
  }

  function serverInFolder(uuid) {
    return allFolders.some(function (f) {
      return folderMemberUUIDs(f).indexOf(uuid) !== -1;
    });
  }
  ...
  function setServerFolder(uuid, folderId) {
    ...
    if (folderId !== null) {
      var target = null;
      allFolders.forEach(function (f) {
        if (String(f.id) === String(folderId)) target = f;
      });
  ...
  function folderCardHtml(folder, idx) {
    ...
    return (
      '<div class="al-card flex items-center gap-3 cursor-pointer relative transition-[background,border-color,box-shadow] select-none hover:border-[var(--theme-border-accent)] group folder-card" role="button" tabindex="0" aria-label="Open folder ' +
      ...
      "'>" +
      alIcon("folder", "h-5 w-5 shrink-0", {
        style: "color:var(--theme-warning);",
      }) +
      '<div class="min-w-0 flex-1">' +
      '<p class="text-sm font-medium truncate" style="color:var(--theme-text-strong);">' +
      escapeHtml(folder.name) +
      "</p>" +
  ...
  function renderServerViews() {
    document
      .querySelectorAll(".al-server-card[data-server-uuid]")
  ...
  function renderFolderPopupContent(memberUUIDs) {
    ...
    var serversIn = allServers.filter(function (s) {
      return memberUUIDs.includes(s.UUID);
    });
    if (serversIn.length === 0) {
      folderPopupContent.innerHTML =
        '<p class="text-sm text-neutral-400 col-span-2">No servers — drag a card here to add one.</p>';
      return;
    }
    serversIn.forEach(function (s) {
      var row = document.createElement("div");
      row.className =
        "flex items-center gap-2 bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-200 dark:border-white/5 rounded-xl px-3 py-2.5 hover:bg-neutral-100 dark:hover:bg-neutral-700/40 transition";
      var running = s.status === "running";
      row.innerHTML =
        '<a href="/server/' +
        s.UUID +
        '" class="flex items-center gap-2 flex-1 min-w-0">' +
        '<span class="text-sm font-medium text-neutral-800 dark:text-white truncate">' +
        escapeHtml(s.name) +
        "</span>" +
        '<span class="ml-auto shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded-md ' +
        (running
            ...
            if (d.success) {
              ...
              if (window.al) {
                window.al.removeRow(row).then(function () {
                  var f = null;
                  allFolders.forEach(function (x) {
                    if (String(x.id) === String(activeFolderId)) f = x;
                  });
                  renderFolderPopupContent(
                    f ? folderMemberUUIDs(f) : memberUUIDs,
                  );
                });
              } else {
  ...
  function renderFolders() {
    var section = document.querySelector("#folderGrid")
      ? document.querySelector("#folderGrid").closest(".mb-8")
      : null;
    ...
    if (!section) {
      ...
      document.querySelectorAll(".al-section-label").forEach(function (l) {
        if (l.textContent === "Servers") serversLabel = l;
      });
      section = document.createElement("div");
      section.className = "mb-8";
      section.innerHTML =
        '<p class="al-section-label">Folders</p><div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3" id="folderGrid"></div><p class="text-xs mt-2" style="color:var(--theme-text-muted);">Drag a server card onto a folder to add it</p>';
      if (serversLabel && serversLabel.parentNode)
        serversLabel.parentNode.insertBefore(section, serversLabel);
  ...
  if (gridView && listView && gridViewBtn && listViewBtn) {
    if (localStorage.getItem("serverViewPreference") === "list")
      switchView("list");
  ...
  newFolderOverlay.addEventListener("click", function (e) {
    if (e.target === newFolderOverlay)
      closeOverlay(newFolderOverlay, newFolderPanel);
  });
  ...
  document
    .getElementById("closeFolderPopup")
    .addEventListener("click", function () {
      closeOverlay(folderPopupOverlay, folderPopupPanel, function () {
  ...
  deleteFolderOverlay.addEventListener("click", function (e) {
    if (e.target === deleteFolderOverlay)
      closeOverlay(deleteFolderOverlay, deleteFolderPanel);
  });
  ...
  confirmDeleteFolder.addEventListener("click", async function () {
    ...
    try {
      var r = await fetch("/api/folders/" + activeFolderId, {
        method: "DELETE",
      });
      var d = await r.json();
      ...
      if (d.success) {
        ...
        allFolders = allFolders.filter(function (f) {
          return String(f.id) !== String(activeFolderId);
        });
        renderFolders();
        renderServerViews();
  ...
  document.querySelectorAll(".server-card[draggable]").forEach(function (card) {
    ...
    card.addEventListener("dragend", function () {
      ...
      document.querySelectorAll(".folder-card").forEach(function (f) {
        f.classList.remove("fc-drag-over");
      });
    ...
    card.addEventListener("contextmenu", function (e) {
      e.preventDefault();
      document.querySelectorAll(".server-ctx-menu").forEach(function (m) {
  ...
  document.querySelectorAll(".ctx-add-to-folder").forEach(function (btn) {
    btn.addEventListener("click", async function (e) {
      ...
      if (allFolders.length === 1) {
        try {
          var r = await fetch("/api/folders/" + allFolders[0].id + "/servers", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  document.querySelectorAll(".ctx-remove-from-folder").forEach(function (btn) {
    btn.addEventListener("click", async function (e) {
      e.stopPropagation();
      try {
        var r = await fetch("/api/folders/servers/" + btn.dataset.uuid, {
          method: "DELETE",
        });
        var d = await r.json();
        if (d.success) {
  ...
  function openFolderPicker(serverUUID) {
    ...
    allFolders.forEach(function (f) {
      ...
      btn.className =
        "flex items-center gap-2.5 w-full text-left bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-200 dark:border-white/5 rounded-xl px-3 py-2.5 hover:bg-neutral-100 dark:hover:bg-neutral-700/40 transition";
      btn.innerHTML =
        alIcon("folder", "h-4 w-4 text-amber-500 shrink-0") +
        '<span class="text-sm text-neutral-800 dark:text-white">' +
        escapeHtml(f.name) +
        "</span>";
      btn.addEventListener("click", async function () {
        try {
          var r = await fetch("/api/folders/" + f.id + "/servers", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ serverUUID: serverUUID }),
          });
          var d = await r.json();
          if (d.success) {
            showToast("Added to folder.", "success");
  ...
  var serverUUIDs = allServers.map(function (s) {
    return s.UUID;
  });
  ...
  function applyServerStatus(uuid, status) {
    ...
    var badge = card.querySelector(
      ".al-badge-online, .al-badge-offline, .al-badge-warning, .al-badge-info",
    );
    if (!badge) return;

    var liveTitle = "Live";
    if (status.updatedAt)
      liveTitle += " · " + new Date(status.updatedAt).toLocaleTimeString();
    ...
    if (status.online) {
      ...
      badge.innerHTML =
        '<span class="al-dot-online"></span> Online' +
        (uptime ? " · " + uptime : "");
  ...
  function onRealtimeStatus(uuid, snap) {
    ...
    var entry = allServers.find(function (s) {
      return s.UUID === uuid;
    });
    if (entry) {
  ...
  function wireRealtime() {
    ...
    function unwatchAll() {
      realtimeDisposers.splice(0).forEach(function (dispose) {
        dispose();
      });
      serverUUIDs.forEach(function (uuid) {
        try {
    ...
    serverUUIDs.forEach(function (uuid) {
      realtimeDisposers.push(
        st.observe("server:status:" + uuid, function (snap) {
          onRealtimeStatus(uuid, snap);
        }),
      );
    });
  ...
  if (onboardingModal) {
    ...
    function renderStep() {
      ...
      dots.forEach(function (d) {
        d.style.background =
          Number(d.dataset.step) <= currentStep
            ? "var(--theme-text)"
